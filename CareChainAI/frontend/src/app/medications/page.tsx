...
export default function MedicationsPage() {
  ...
  const fetchMeds = () => {
    fetch("http://10.166.189.194:8000/api/medications/", { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    await fetch("http://10.166.189.194:8000/api/medications/", {
      method: "POST",
  ...
  const toggleActive = async (id: number) => {
    const res = await fetch(`http://10.166.189.194:8000/api/medications/${id}/toggle`, {
  ...
  const deleteMed = async (id: number) => {
    await fetch(`http://10.166.189.194:8000/api/medications/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
