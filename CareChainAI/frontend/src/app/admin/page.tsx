...
export default function AdminPage() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const [statsRes, patientsRes, reportsRes] = await Promise.all([
        fetch("http://10.166.189.194:8000/api/admin/stats",    { headers }),
        fetch("http://10.166.189.194:8000/api/admin/patients", { headers }),
        fetch("http://10.166.189.194:8000/api/admin/reports",  { headers }),
      ]);
  ...
  const handleDeleteUser = async (id: number) => {
    setDeleting(id);
    try {
      await fetch(`http://10.166.189.194:8000/api/admin/users/${id}`, {
        method: "DELETE",
