...
export default function AppointmentsPage() {
  ...
  const fetchAppts = () => {
    fetch("http://10.166.189.194:8000/api/appointments/", { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    await fetch("http://10.166.189.194:8000/api/appointments/", {
      method: "POST",
  ...
  const updateStatus = async (id: number, status: string) => {
    const res = await fetch(`http://10.166.189.194:8000/api/appointments/${id}/status?status=${status}`, {
  ...
  const deleteAppt = async (id: number) => {
    await fetch(`http://10.166.189.194:8000/api/appointments/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
