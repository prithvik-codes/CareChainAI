...
export default function VitalsPage() {
  ...
  const fetchVitals = () => {
    fetch("http://10.166.189.194:8000/api/vitals/", { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleSave = async () => {
    ...
    await fetch("http://10.166.189.194:8000/api/vitals/", {
  ...
  const handleDelete = async (id: number) => {
    await fetch(`http://10.166.189.194:8000/api/vitals/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
